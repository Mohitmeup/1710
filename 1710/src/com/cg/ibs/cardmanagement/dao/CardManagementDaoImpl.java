...
public class CardManagementDaoImpl implements CustomerDao, BankDao {
	...
	public void actionUpgradeCC(String queryId) throws IBSException {
		...
		try (Connection connection = ConnectionProvider.getInstance().getConnection();)
		{
			try(PreparedStatement preparedStatement = connection.prepareStatement(SqlQueries.UPGRADE_CREDIT_CARD);){
			preparedStatement.setString(1, caseIdObj.getDefineQuery());
			preparedStatement.setBigDecimal(2, new BigDecimal(caseIdObj.getCardNumber()));
			preparedStatement.executeQuery();
		}
		}catch(SQLException| IOException exception) {
			throw new IBSException(ErrorMessages.CARD_MESSAGE);
		}
//		caseIdObj = queryDetails.get(queryId);
//		BigInteger creditCardNumber = caseIdObj.getCardNumber();
//		String type = caseIdObj.getDefineQuery();
